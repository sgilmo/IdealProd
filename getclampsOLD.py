...
def update_db_fm(dbase):
    """ Add unfiltered part data to SQL server database from FileMaker"""
    dbcnxn = pyodbc.connect(CONNECTION)
    cursor = dbcnxn.cursor()
    cursor.fast_executemany = True
    start = timer()
    try:
        cursor.execute("TRUNCATE TABLE production.parts_fm")
        dbcnxn.commit()
    except Exception as e:
        msg = 'MSSQL Table Deletion Failed: ' + str(e) + " [" + update_db.__name__ + "]"
        logger.error(msg)
        ...
        print("Delete Time = " + str(round((timer() - start), 3)) + " sec")
    # Load part data onto SQL server
    sql = """INSERT INTO production.parts_fm (PartNumber,Band,Housing,Screw,Feed,
                    DiaMax,DiaMin,HexSz,BandThickness,BandWidth,CamInspect,ScrDrvChk)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?);"""
    try:
        start = timer()
        ...
        msg = 'MSSQL Table Update Failed: ' + str(e) + " [" + update_db_fm.__name__ + "]"
        logger.error(msg)
        print(msg)
    else:
        print(str(len(dbase)) + " MSSQL Records Processed in " + str(round((timer() - start), 3)) + " sec")
